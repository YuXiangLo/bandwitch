...
const networkData = JSON.parse(fs.readFileSync('nic_info.json'));

networkData.forEach(interfaceInfo => {
...
async function updateUserNICs (username, nic, action, ip_last){
    ...
    data=fs.readFileSync(filepath)
		...
		fs.writeFile(filepath, JSON.stringify(user, null, 2), (err,data)=>{console.error(err); return;})
...
app.post('/turn_on', async (req, res) => {
	...
	if(nics)
		update_IP_table(ip_last,nics,res);
});
...
const update_IP_table = (ip_last, nics, res) => {
	...
	for (let i = 0; i < nics.length; i++) {
		command += ` nexthop via ${networkDict[nics[i]].gateway} dev ${nics[i]} weight ${1}`;
	}
